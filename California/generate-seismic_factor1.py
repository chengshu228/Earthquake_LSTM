print('\n\tBegin generate-seismic_factor1.py')
...
min_longitude, max_longitude = config.min_longitude, config.max_longitude
min_latitude, max_latitude = config.min_latitude, config.max_latitude
...
min_lat, mid_lat, max_lat = min_latitude, max(latitude)-span_lat, max_latitude
min_lon, mid_lon, max_lon = min_longitude, max(longitude)-span_lon, max_longitude
                ...
                for key2, lon in enumerate(np.arange(min_lon, max_lon-span_lon+0.1, 4-key1*0.5)):
                    ...
                    if datetime.datetime(y,m,1,0,0,0) + \
                        ...
                        if not (mag_next.size>0 and mag.size>0 and len(mag)>=min_number):
                            print('\t\t\t', 'Warning: ', 'next:({}),'.format(mag_next.size), mag_next.size>0, \
                                ' min_number>=30:({})'.format(len(mag)), len(mag)>=30)
                        else:
