...
if n_splits != 1:
    ...
    x_train = input_data[:(num1+num2)-len_train_val, :]
    y_train = output_data[:(num1+num2)-len_train_val, :]
...
y_train = y_train.reshape(-1, output_node)
y_val = y_val.reshape(-1, output_node)
...
diff_train = (y_train_pre-y_train).reshape(-1, output_node)
diff_val = (y_val_pre-y_val).reshape(-1, output_node)
diff_test = (y_test_pre-y_test).reshape(-1, output_node)
...
if loc_block <= 10:
    location_block = label = [loc_block]
elif loc_block == 55:
    location_block = label = np.arange(1, 5+1, 1)
elif loc_block == 66:
    location_block = label = np.arange(0, 5+1, 1)

if loc_block == 55 or loc_block == 66:
    label += 1
...
for i, threshold in enumerate([6, 7]):
    ...
    plt.text(0.015, 1.1, '调和平均数 F1_Score = {:.2f}'.format(F1_Score), va='bottom', fontsize=14, fontdict=fontcn)
    plt.text(0.015, 1.0, '综合评分 R_Score = {:.2f}'.format(R_Score), va='bottom', fontsize=14, fontdict=fontcn)
    plt.text(0.015, 0.9, '{}'.format('-'*26), va='bottom', fontsize=14, fontdict=fontcn)
...
for i, threshold in enumerate([6, 7]):
    ...
    ax.axes.xaxis.set_ticks([])
    ax.axes.yaxis.set_ticks([])
plt.suptitle(r'Test Set', fontdict=fonten, fontsize=20, color='red')
plt.tight_layout()
plt.savefig(r'.\figure\{}-ConfuseMatrix.png'.format(filename))
plt.show()
...
plt.text(7.2, 7.2, 'y=x', fontdict=fonten, fontsize=16)
if n_splits == 1:
...
if len(fake_test)-len(miss_test) >= 0:
    plt.text(0.02, 0.02, '虚报比漏报多{}次'.format(len(fake_test)-len(miss_test)), fontsize=16, fontdict=fontcn)
else:
    plt.text(0.02, 0.02, '虚报比漏报少{}次'.format(len(miss_test)-len(fake_test)), fontsize=16, fontdict=fontcn)
plt.ylim(0.015, 0.07)
plt.title(r'Testing Set', fontdict=fonten, fontsize=20, color='red')
ax = plt.gca()
ax.axes.xaxis.set_ticks([])
...
if len(fake)-len(miss) >= 0:
    plt.text(0.02, 0.02, '虚报比漏报多{}次'.format(len(fake)-len(miss)), fontsize=16, fontdict=fontcn)
else:
    plt.text(0.02, 0.02, '虚报比漏报少{}次'.format(len(miss)-len(fake)), fontsize=16, fontdict=fontcn)
plt.ylim(0.015, 0.07)
plt.title(r'All Set', fontdict=fonten, fontsize=20, color='red')
ax = plt.gca()
ax.axes.xaxis.set_ticks([])
