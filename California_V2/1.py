...
with open(data_location+f'\\date-{file_name}1.txt', \
...
# 2*4
with open(data_location+f'\\factor-{file_name}1.txt', \
    mode='w+', encoding='utf-8') as fout:
    for y in np.arange(min_year, max_year+1):
        print('\t\tyear:', y)
        for m in np.arange(1, 12+1):
                ...
                for key2, lon in enumerate(np.arange(min_lon, max_lon-span_lon+0.1, 4-key1*0.5)):
                    ...
                    if datetime.datetime(y,m,1,0,0,0) + \
                        ...
                        if not (mag.size>0 and len(mag)>=min_number):
                            ...
                            fout.write('{0:.1f} {1:.0f} {2:.1f} {3:.2f} {4:.4f} {5:.4f} {6:.4f} {7:.4f} {8:.4f} {9:.2f} {10:.4f} {11:.4f} {12:.4f} {13:.4f} {14:.4f} {15:.4f} {16:.4f} {17:}\n'.\
                                ...
                                k, epicenter_lon, epicenter_lat,n_lstsq[0]))  
