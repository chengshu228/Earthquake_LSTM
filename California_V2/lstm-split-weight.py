...
input_data = np.concatenate((
    factor[:-blocks, 0].reshape(-1, 1),  # frequency
    factor[:-blocks, 1].reshape(-1, 1),  # frequency
    factor[:-blocks, 2].reshape(-1, 1),  # max_magnitude
    factor[:-blocks, 3].reshape(-1, 1),  # mean_magnitude
    factor[:-blocks, 4].reshape(-1, 1), # b_lstsq
    factor[:-blocks, 5].reshape(-1, 1),  # b_mle
    factor[:-blocks, 6].reshape(-1, 1), # a_lstsq
    factor[:-blocks, 7].reshape(-1, 1), # max_mag_absence
    factor[:-blocks, 8].reshape(-1, 1), # rmse_lstsq
    factor[:-blocks, 9].reshape(-1, 1), # total_energy_square
    factor[:-blocks, 10].reshape(-1, 1), # mean_lon
    factor[:-blocks, 11].reshape(-1, 1), # rmse_lon
    factor[:-blocks, 12].reshape(-1, 1), # mean_lat
    factor[:-blocks, 13].reshape(-1, 1), # rmse_lat
    factor[:-blocks, 14].reshape(-1, 1), # k
    factor[:-blocks, 15].reshape(-1, 1), # epicenter_longitude
    factor[:-blocks, 16].reshape(-1, 1), # epicenter_latitude
    ), axis=1)
input_data = input_data.reshape(-1, blocks*(features*n+m))
# input_data = input_data[:,:features]
output_data = factor[blocks:,0].reshape(-1,blocks) 
...
model.compile(optimizer=optimizer, 
    ...
    metrics=[tf.keras.metrics.RootMeanSquaredError(name='rmse')], sample_weight_mode='temporal')


# # Calculate the weights for each class so that we can balance the data
# weights = sample_weight.compute_class_weight('balanced',
#            np.unique(y_train),
#            y_train)
...
adjustedWeights = np.ones(shape=(len(y_train),))
print(adjustedWeights)
for i in np.arange(len(y_train)):
    adjustedWeights[i] = 10 ** (1.5*y_train[i,0]+11.18)
#     np.around(np.sqrt(np.power(10, 1.5*output_data+11.8)), 0)
adjustedWeights=np.around(np.sqrt(np.power(10, 1.5*y_train+11.8)), 0)

# sample_weight[y_train == 3] = 1.5

# adjustedWeights = np.around(np.sqrt(np.power(10, 1.5*output_data+11.8)), 0)
...
# for i in np.arange(len(y_train)):
    ...
    #     callbacks=utils.checkpoints(model_name,file_name), class_weight=adjustedWeights[i])
...
history = model.fit(x_train, y_train, 
        ...
        batch_size=batch_size, 
        verbose=2, shuffle=False, 
        callbacks=[tf.keras.callbacks.ModelCheckpoint(file_location, verbose=2,  
		monitor='loss', save_best_only=True, save_weights_only=False),
		tf.keras.callbacks.EarlyStopping(monitor='loss', patience=100, verbose=True)],
        # utils.checkpoints(model_name,file_name), 
        sample_weight=adjustedWeights)
        # np.array([w0 if r==0 else w1 for r in y]))
...
#     # batch_size=batch_size, 
#     verbose=2, shuffle=False, 
# 	callbacks=utils.checkpoints(model_name,file_name), class_weight=adjustedWeights)
...
# model.save(model_dir+f'\layer{layer:.0f}-layer_size{layer_size:.0f}-loc_block{loc_block:.0f}.tf')

# model = tf.keras.models.load_model(
#     model_dir+f'\layer{layer:.0f}-layer_size{layer_size:.0f}-loc_block{loc_block:.0f}.tf')
...
plt.xlim(min(x_label1)+2500, max(x_label1)-2500)
