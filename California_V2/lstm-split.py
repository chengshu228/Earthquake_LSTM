...
factor = np.genfromtxt(file, delimiter=' ')
...
input_data, output_data = utils.lockback(factor, lockback_samples=config.lockback_samples)
...
x_train = x_scaler.fit_transform(x_train.reshape(-1, 16*config.lockback_samples)).reshape(-1, config.lockback_samples, 16)
x_test = x_scaler.transform(x_test.reshape(-1, 16*config.lockback_samples)).reshape(-1, config.lockback_samples, 16)
...
optimizer = optimizers.Adam(learning_rate=learning_rate, epsilon=1e-08, decay=0.0) 
